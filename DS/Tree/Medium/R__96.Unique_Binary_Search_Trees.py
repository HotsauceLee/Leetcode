...
# Time: O(n^2)
# Space:
# Running Time: Limit exceeded, very large
class Solution(object):
    def numTrees(self, n):
        ...
        for i in range(n):
            root_val = i + 1
            left_unique = self.numTrees(root_val - 1)
            right_unique = self.numTrees(n - root_val)
            result += left_unique*right_unique
...
#========== Catalan Number ===============
# https://discuss.leetcode.com/topic/13321/a-very-simple-and-straight-ans-based-on-math-catalan-number-o-n-times-o-1-space/2
# Time: O(n)
# Space: O(1)
# Running time:
int numTrees(int n) {
